...
def main():
    ...
    parser.add_argument(
        "--toy", action="store_true", help="Download toy examples instead of full data"
    )
    parser.add_argument(
        "--start", type=int, default=1, help="Start batch index (full data only)"
    )
    parser.add_argument(
        "--end", type=int, help="End batch index inclusive (full data only)"
    )
    parser.add_argument(
    ...
    patterns = (
        None
        if args.toy or args.end is None
        else [f"*batch_{i:05d}*" for i in range(args.start, args.end + 1)]
    )

    snapshot_download(
        repo, repo_type="dataset", allow_patterns=patterns, local_dir=args.output
    )
